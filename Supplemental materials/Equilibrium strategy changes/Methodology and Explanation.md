...
The code assembles all three displays from fingerprint-checked saved calculations. `LitigCharts equilibrium-manuscript --input <Equilibrium strategy changes> --article <article repository>` refreshes numbered Tables 2 and 3 with the short selections and updates their source manifest. The supplemental scheduler generates all three canonical displays after the seven individual tables. Full packet files and qualifying coordinates remain available online.
...
The risk-averse manuscript selection, `selected-risk-averse-strategy-mechanisms`, contains eleven displayed rows: American-rule filing at signals 0.05-0.15 and the offsetting effects at 0.35, the continuing plaintiff demand at 0.35, and continuing defendant offers at 0.05-0.15 and 0.65; the Trial-rule plaintiff exit commitment at 0.45 and defendant exit commitments at 0.45 and 0.55; the Complete-rule continuing demand at 0.45-0.55; and the Trial-to-Complete answering response at 0.75 and 0.85-0.95. The first three panels change risk preferences; the last changes the fee rule under risk aversion. Every selected history is reached in both endpoint equilibria and all conditional comparisons are defined. Some intermediate best responses avoid the selected histories; their conditional comparisons and reach diagnostics remain in the source JSON and full packet. Opponent-exit contributions are retained, remaining contributions are zero, and the display has no sensitivity column or asterisks.
