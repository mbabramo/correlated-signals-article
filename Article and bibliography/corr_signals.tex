...
Under the American rule, each party bears its own costs. Trial Fee-Shifting requires the trial loser to reimburse the winner's incurred costs. Complete Fee-Shifting adds reimbursement of the plaintiff's filing cost when the defendant does not answer, and reimbursement of the opponent's incurred costs when a party later exits unilaterally. No fee award attaches to nonfiling, and withdrawal does not incur future trial expenses. In settled cases, each party bears its own costs. Table \ref{tab:model-primitives} summarizes the model primitives and fee rules.
...
Figure \ref{fig:dispositions} reports the dispositions of potential disputes under the three fee-shifting rules with risk neutrality and ordinary costs. Figure \ref{fig:participation-and-offers} then shows the participation and bargaining strategies underlying these outcomes.
...
Table \ref{tab:strategy-mechanisms} presents selected decompositions of the changes in equilibrium strategies under the fee-shifting rules.
...
Table \ref{tab:risk-averse-outcomes} reports dispositions and monetary outcomes when both parties have CARA risk aversion with coefficient $\alpha=2$, holding ordinary costs and the information structure fixed. These results describe the selected equilibria; Section \ref{sec:multiple-equilibria} considers variation across recovered equilibria.

\begin{table}[H]
    \centering
    \caption{Risk-averse dispositions and welfare outcomes}
    \label{tab:risk-averse-outcomes}
    \includegraphics[width=\linewidth]{../Tables/Table 3 - Risk-averse outcomes.pdf}
    \par\smallskip
    \begin{minipage}{\linewidth}
    \footnotesize\setstretch{1}
    \textit{Note:} All outcomes use the population of potential disputes. Mutual exit is allocated equally to abandonment and default. Welfare measures are amounts per potential dispute in units of damages, rounded to three decimals; they are distinct and should not be added together.
    \end{minipage}
\end{table}
...
Table \ref{tab:welfare-outcomes} reports the risk-neutral welfare measures. Together with the risk-averse measures in Table \ref{tab:risk-averse-outcomes}, it permits comparison of monetary burdens, gross outcome error, and real expenditures across fee rules and risk preferences, using the definitions in Section \ref{sec:outcome-measures}.

\begin{table}[H]
    \centering
    \caption{Risk-neutral welfare outcomes}
